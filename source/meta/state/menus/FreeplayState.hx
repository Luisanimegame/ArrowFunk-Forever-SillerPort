...
class FreeplayState extends MusicBeatState
{
	...
	private var chess:FlxBackdrop;
	private var gradient:FlxSprite;
	private var menushit:FlxSprite;
	...
	override function create()
	{
		...
		**/
		...
		// LOAD CHARACTERS
		bg = new FlxSprite().loadGraphic(Paths.image('menus/arrow/menuBG'));
		bg.x -= 10;
		add(bg);
		...
		gradient = new FlxSprite().loadGraphic(Paths.image('menus/arrow/otherassets/freeplay/fpbgradient'));
		gradient.antialiasing = true;
		add(gradient);

		menushit = new FlxSprite(0).loadGraphic(Paths.image('menus/arrow/otherassets/freeplay/fpshit'));
		...
		overlay = new FlxSprite().loadGraphic(Paths.image('menus/arrow/otherassets/freeplay/fpov3'));
	...
	public function addSong(songName:String, weekNum:Int, songCharacter:String, songColor:FlxColor)
	{
		...
		switch (songName.toLowerCase()) {
			case 'earthquake':
				songColor = FlxColor.fromRGB(46, 93, 115);
			case '-debug':
				songColor = FlxColor.fromRGB(66, 53, 135);
		}
