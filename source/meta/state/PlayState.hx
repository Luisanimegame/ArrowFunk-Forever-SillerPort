...
class PlayState extends MusicBeatState
{
	...
	public static var assetModifier:String = 'arrow';
	...
	//interruptor
	private var darkBalada:Bool = false;

	//virus
	public static var goofyGoo:Int = 0;
	//public static var grêmio:Int = 9029049404595021;
	...
	override public function create()
	{
		...
		// default song
		if (SONG == null)
			SONG = Song.loadFromJson('-debug', '-debug');
		...
		{
			case 'faveladia':
				add(stageBuild.favelalight);
			case 'favela':
				add(stageBuild.favelalight);
				add(stageBuild.favelight);
				add(stageBuild.kleitin);
				add(stageBuild.daniel);
			case 'favelanoite':
				add(stageBuild.kleitin);
				add(stageBuild.daniel);
				add(stageBuild.pessoas);
				add(stageBuild.favelalight);
			case 'floresta':
				add(stageBuild.treefront);
				add(stageBuild.florestalight);
		}

		if(curStage == 'baladamedovirus')
		{
			var acidScreen:FlxSprite = new FlxSprite().loadGraphic(Paths.image('backgrounds/baladamedovirus/screen'));
			acidScreen.setGraphicSize(Std.int(acidScreen.width * 2));
			acidScreen.updateHitbox();
			acidScreen.screenCenter(XY);
			acidScreen.antialiasing = true;
			add(acidScreen);
	...
	function startCaching()
	{
		...
		// cache songs
		Paths.music('Hot_Lunch');
		// in case fnf is still real.
	...
	override public function update(elapsed:Float)
	{
		...
		if (curStage == 'balada') {
			if (darkBalada == true) //Pode me xingar o quanto quiser, o importante é que funciona
			{
				//claro
				stageBuild.fundo1.alpha = 0;
				stageBuild.chao1.alpha = 0;
				stageBuild.base1.alpha = 0;
				stageBuild.luzes1.alpha = 0;
				stageBuild.curti1.alpha = 0;
				stageBuild.balight.alpha = 1;
			} else
			{
				//escuro
				stageBuild.fundo1.alpha = 1;
				stageBuild.chao1.alpha = 1;
				stageBuild.base1.alpha = 1;
				stageBuild.luzes1.alpha = 1;
				stageBuild.curti1.alpha = 1;
				stageBuild.balight.alpha = 0;
			}
		}
		...
		if (goofyGoo > 0) {
			new FlxTimer().start(Std.int(10), function(gooCooldown:FlxTimer)
			{
				goofyGoo -= 1;
			});
		}
		...
		if (!inCutscene) {
			...
			if (health <= 0 || controls.RESET && startedCountdown)
			{
				...
				{
					FlxG.sound.play(Paths.sound('fnf_loss_sfx' + pixelSuffix));
				});
				...
				strumline.allNotes.forEachAlive(function(daNote:Note)
				{
					...
					if (!daNote.tooLate && daNote.strumTime < Conductor.songPosition - (Timings.msThreshold) && !daNote.wasGoodHit)
					{
						if ((!daNote.tooLate) && (daNote.mustPress)) {
							if (!daNote.isSustainNote && daNote.noteType != 1)
							{
								daNote.tooLate = true;
								...
								vocals.volume = 0;
								missNoteCheck(true, daNote.noteData, boyfriend, true);
								// ambiguous name
					...
					if ((((!Init.trueSettings.get('Downscroll'))
						&& (daNote.y < -daNote.height)) || ((Init.trueSettings.get('Downscroll'))
	...
	function changeCharacter(x:Float, y:Float, newChar:String, charName:String)
	{
		var charType:Int = 0;
		switch(charName) {
			...
			case 'gf' | 'girlfriend':
				charType = 2;
			case 'boyfriend' | 'bf':
				charType = Std.parseInt(charName);
		...
		switch(charType) {
			case 0:
				boyfriend.setCharacter(x, y, newChar);
				uiHUD.iconP1.updateIcon(newChar, true);
				uiHUD.reloadHealthBarColors();
				if(stageBuild.playerLoader != null)
				{
					remove(stageBuild.playerLoader);
					Paths.clearUnusedMemory();
				}
			case 1:
				dadOpponent.setCharacter(x, y, newChar);
				uiHUD.iconP2.updateIcon(newChar, false);
				uiHUD.reloadHealthBarColors();
				if(stageBuild.opponentLoader != null)
				{
					remove(stageBuild.opponentLoader);
					Paths.clearUnusedMemory();
				}
			case 2:
				gf.setCharacter(x, y, newChar);
				if(stageBuild.gfLoader != null)
				{
					remove(stageBuild.gfLoader);
					Paths.clearUnusedMemory();
				}
		}
	}
	...
	function goodNoteHit(coolNote:Note, character:Character, characterStrums:Strumline, ?canDisplayJudgement:Bool = true)
	{
		if (!coolNote.wasGoodHit)
		{
			...
			if (coolNote.noteType != 1) {
				...
				// eu sei que não é miss mas vai tomar no cu eu não vou fazer outra função pra perder vida
				decreaseCombo(popMiss);
				doThenDance('damage', boyfriend, 0.4);

				if (health > 0.5)
					goofyGoo += 1;

				//afetando o grêmio
				//grêmio -= 9029049404595021;
			}
			...
			if (canDisplayJudgement) {
				...
				if (!coolNote.isSustainNote && coolNote.noteType != 1) {
					...
					if (coolNote.parentNote != null) {
						...
						if (coolNote.mustPress && !boyfriendStrums.autoplay)
	...
	function characterPlayAnimation(coolNote:Note, character:Character)
	{
		...
		if (coolNote.noteAlt > 0 || coolNote.noteType == 3)
			altString = '-alt';
		...
		switch(coolNote.noteType)
		{
			case 2:
				altString = '-shoot';
				doThenDance('scared', gf, 0.6);
				if (curStage == 'favela' || curStage == 'favelanoite')
				{
					switch(stageBuild.kleistate)
					{
						case 2:
							stageBuild.kleitin.animation.play('susto');
						case 3:
							stageBuild.kleitin.animation.play('bala');
					}
				}
			case 4:
				altString = '-dodge';
			...
			switch(coolNote.noteType)
			{
				case 2:
					...
					if (curStage == 'favela' || curStage == 'favelanoite')
					{
						...
						{
							case 2:
								stageBuild.kleitin.animation.play('susto', true);
							case 3:
								stageBuild.kleitin.animation.play('bala', true);
						}
					}
				case 4:
					altString = '-dodge';
	...
	function startSong():Void
	{
		...
		if (Init.trueSettings.get('Display Timer'))
			FlxTween.tween(uiHUD.timeTxt, {alpha: 1}, 0.5, {ease: FlxEase.circOut});

		FlxTween.tween(uiHUD.songInfo, {x: 0}, 2.6, {ease: FlxEase.expoOut, onComplete: function(twn:FlxTween)
		{
			new FlxTimer().start(2, function(tmr:FlxTimer)
			{
				FlxTween.tween(uiHUD.songInfo, {x: -500}, 2.6, {
					ease: FlxEase.expoIn,
					onComplete: function(twn:FlxTween)
					{
						remove(uiHUD.songInfo);
					}
				});
			});
		}});
		...
		switch (curSong.toLowerCase())
		{
			case 'reboop':
				if (!Init.trueSettings.get('Reduced Movements'))
				{
					switch (curStep)
					{
						case 128, 192, 256, 576, 640:
							defaultCamZoom = 0.6;

						case 184, 248, 568, 632:
							defaultCamZoom = 0.65;

						case 186, 250, 570, 633:
							defaultCamZoom = 0.7;

						case 190, 254, 573, 637:
							defaultCamZoom = 0.75;
					}
				}
				...
				switch (curStep)
				{
					case 128, 256, 416, 512, 544, 640, 656:
						// os dois na velocidade normal
						doThenDance('hey', boyfriend, 0.6);
						doThenDance('cheer', gf, 0.6);
						
					case 192, 320, 448, 480, 576, 608, 672, 687, 704, 719, 735, 752, 831:
						// velocidade normal
						doThenDance('cheer', gf, 0.6);

					case 380, 382, 764, 766:
						// rápido
						doThenDance('cheer', gf, 0.2);
				}
			...
			case 'fresher':
				...
				{
					case 427:
						if (gf.animOffsets.exists('shock'))
							gf.playAnim('shock', true);
						gf.animationDisabled = true;

					case 429:
						gf.animationDisabled = false;
			...
			case 'rap-king':
				if (health >= 1)
				{
					if (curStep == 691) doThenDance('hey', boyfriend, 0.6);
					if (curStep == 2914) FlxG.sound.play(Paths.sound('micthrow'));
				}
				...
				if(!Init.trueSettings.get('Reduced Movements'))
				{
					switch (curStep)
					{
						case 32, 96, 672, 1204, 1600, 1856:
							defaultCamZoom = 0.6;

						case 48, 112, 1104, 1168:
							defaultCamZoom = 0.62;

						case 64, 128, 640, 1144, 1212, 1472, 1728, 2112, 2144, 2176, 2208, 2240, 2272, 2304, 2336, 2368:
							defaultCamZoom = 0.57;

						case 688, 692, 1120, 1184, 1968:
							defaultCamZoom = 0.65;   
							
						case 698, 704, 1200, 1976: 
							defaultCamZoom = 0.7; 
				...
				switch (curStep)
				{
					...
					case 960:
						...
						darkBalada = true;
						changeCharacter(-40, 840, 'sagu-fire-dark', 'dadOpponent');
						changeCharacter(295, 90, 'barbara-dark', 'girlfriend');
						changeCharacter(815, 840, 'bidu-dark', 'bf');

					case 1215:
						set_songSpeed(2.8);
						FlxG.camera.flash(FlxColor.WHITE, 1.5);
						darkBalada = false;
						changeCharacter(-40, 840, 'sagu-fire', 'dadOpponent');
						changeCharacter(295, 90, 'barbara', 'girlfriend');
						changeCharacter(815, 840, 'bidu', 'bf');
			...
			case 'bittersweet':
				switch (curStep)
				{
					...
					case 1:
						defaultCamZoom = 0.7;
					case 256:
						stageBuild.spookersvel = 1;
						FlxTween.tween(stageBuild.dancef, {alpha:1}, 2.4, {ease: FlxEase.expoOut});
					case 672:
						defaultCamZoom = 0.63;
					case 1040, 1296:
						stageBuild.spookersvel = 1;
					case 336, 1024, 1056:
						stageBuild.spookersvel = 2;

					// YAY!
					case 668:
						if (dadOpponent.animOffsets.exists('hey'))
							dadOpponent.playAnim('hey', true);
						dadOpponent.animationDisabled = true;
						defaultCamZoom = 0.75;

					case 670:
						dadOpponent.animationDisabled = false;

					// YEAH!!!
					case 732:
						if (boyfriend.animOffsets.exists('hey'))
							boyfriend.playAnim('hey', true);
						boyfriend.animationDisabled = true;
						defaultCamZoom = 0.75;

					case 734:
						boyfriend.animationDisabled = false;
			...
			case 'nightfall':
				if(!Init.trueSettings.get('Reduced Movements'))
				{
					switch (curStep)
					{
						case 640, 880, 1664, 2688:
							stageBuild.dancefvel = 2;
						case 880, 1664, 2176:
							defaultCamZoom = 0.65;
				...
				switch (curStep)
				{
					case 1:
						FlxTween.tween(stageBuild.dancef, {alpha:1}, 2.4, {ease: FlxEase.expoOut});
					case 640, 1664:
						stageBuild.dancefvel = 3;
					case 880, 1152, 2176:
						stageBuild.dancefvel = 2;
					case 896, 1920, 2432:
						stageBuild.dancefvel = 1;
					case 2160:
						changeCharacter(-130, 828, 'kids-happy', 'dadOpponent');
					case 2668:
						stageBuild.dancefvel = 3;
						FlxTween.tween(stageBuild.dancef, {alpha:0}, 3.4, {ease: FlxEase.expoIn});
				}
			...
			case 'shacklesz':
				if(!Init.trueSettings.get('Reduced Movements'))
				{
					if (curStep > 1135 && curStep < 1150) defaultCamZoom -= 0.005;
					switch(curStep)
					{
						case 128, 960, 992, 1018, 1280:
							defaultCamZoom = 0.85;
						case 384, 832, 896, 1024, 1088, 1476:
							defaultCamZoom = 0.8;
						case 636, 898:
							defaultCamZoom = 0.7;
						case 640, 1408, 1536:
							defaultCamZoom = 0.75;
						case 640, 660, 666, 672, 692, 698, 704, 724, 730, 736, 756, 762, 768, 788, 794, 800, 806, 812, 820, 826, 832, 852, 858, 864, 876, 884, 890:
			...
			case 'blam':
				...
				{
					case 1:
						stageBuild.kleistate = 1;
					case 624:
						doThenDance('drink', gf, 2, true);
					case 640:
						dadOpponent.setCharacter(-130, 815, 'dono-gun');
						changeCharacter(-130, 815, 'dono-gun', 'dadOpponent');
						doThenDance('spit', gf, 1);
						gf.animationDisabled = true;
						new FlxTimer().start(1.15, function(danceTimer:FlxTimer)
						{
							gf.animationDisabled = false;
							changeCharacter(325, 115, 'barbara-chocolate-sunset-scared', 'gf');
						});
					case 752:
						FlxTween.tween(stageBuild.kleitin, {x: 1080}, 3.8, {
							startDelay: 0.1,
							ease: FlxEase.linear,
							onComplete: function(twn:FlxTween)
							{
								stageBuild.kleitin.animation.play('stop', true);
								stageBuild.kleistate = 2;
							}
						});
					case 1016:
						FlxTween.tween(stageBuild.carrofoda, {x:-600}, 2, {ease: FlxEase.quartOut});
					case 1116:
						FlxTween.tween(stageBuild.danielzinho, {x:-1020}, 8, {ease: FlxEase.linear,
						onComplete: function(twn:FlxTween)
						{
							remove(stageBuild.danielzinho);	
						}});
					case 1424:
						FlxTween.tween(stageBuild.daniel, {x:-540}, 1.4, {ease: FlxEase.sineOut});
					case 1865:
						stageBuild.busao.y = -45;
						FlxTween.tween(stageBuild.busao, {x:230}, 6, {ease: FlxEase.quartOut}); //BUSAO EVENT //eu so não coloquei o y junto com o x no tween pois o ease é diferente então me, não vou mexer nisso.
						FlxTween.tween(stageBuild.busao, {y:-40}, 6, {ease: FlxEase.bounceInOut});
					case 1904:
						stageBuild.kleistate = 3;
					case 2488:
						if (dadOpponent.animOffsets.exists('fail'))
							dadOpponent.playAnim('fail');
						FlxG.sound.play(Paths.sound('oops'));
					case 2492:
						if (dadOpponent.animOffsets.exists('out'))
							dadOpponent.playAnim('out');
						FlxG.sound.play(Paths.sound('oops'));

				}
				if(!Init.trueSettings.get('Reduced Movements'))
				{
					if (curStep > 1 && curStep < 129) defaultCamZoom += 0.003;
					switch(curStep)
					{
						case 16, 128, 640, 1024, 1664, 1728, 1792, 1856, 2016, 2688:
							defaultCamZoom = 0.7;
						case 32:
							defaultCamZoom = 0.72;
						case 48:
						...
						case 80:
							defaultCamZoom = 0.78;
						case 96, 512, 768, 1280, 1382, 1398, 1510, 1526, 1776, 1920, 2176:
							defaultCamZoom = 0.8;
						case 112:
							defaultCamZoom = 0.82;
						case 116:
						...
						case 124:
							defaultCamZoom = 0.88;
						case 386, 766, 1152, 1536, 1712, 1824, 2048, 2432, 2496:
							defaultCamZoom = 0.75;
						case 624, 2304, 2486:
							defaultCamZoom = 0.85;
						case 628, 1522, 1380, 1396, 1506, 1522:
							defaultCamZoom = 0.9;
						case 632:
							defaultCamZoom = 0.95;
					}
					if (curStep >= 2008 && curStep < 2016) defaultCamZoom -= 0.002;
				}
			case 'loaded':
				switch(curStep)
				{
					case 607:
						changeCharacter(-130, 815, 'dono-doublegun-night', 'dadOpponent');
						if (dadOpponent.animOffsets.exists('mic'))
							dadOpponent.playAnim('mic');
						dadOpponent.animationDisabled = true;
					case 628:
						dadOpponent.animationDisabled = false;
						if (dadOpponent.animOffsets.exists('hey'))
					...
					case 629:
						changeCharacter(792, 770, 'bidu-gold-night-alt', 'bf');
				}
				if(!Init.trueSettings.get('Reduced Movements'))
				{
					switch(curStep)
					{
						case 128, 192, 288, 320, 368, 696, 766, 832, 896, 1280, 1588, 1652:
							defaultCamZoom = 0.9;
						case 144, 156, 208, 224, 380, 1596, 1660:
							defaultCamZoom = 0.8;
						case 148, 184, 212, 248, 1152, 1408, 1592, 1600, 1656:
							defaultCamZoom = 0.85;
						case 150, 156, 214, 220:
							defaultCamZoom = 0.7;
						case 154, 218, 384, 1664:
							defaultCamZoom = 0.75;
						case 312, 354, 374, 640, 704, 768, 864, 1024, 1344, 1568, 1632:
							defaultCamZoom = 0.95;
						case 820, 884, 1088, 1376:
							defaultCamZoom = 1;
						case 824, 888, 1400:
							defaultCamZoom = 1.05;
						case 828, 892:
					...
					if (curStep >= 112 && curStep < 127) defaultCamZoom += 0.002;
				}
			case 'earthquake':
				if(!Init.trueSettings.get('Reduced Movements')) {
					switch (curStep) {
				...
				switch (curStep) {
					case 1248:
						// não vou usar o "doThenDance" aqui porque não precisa
						if (dadOpponent.animOffsets.exists('tampa'))
							dadOpponent.playAnim('tampa');
						FlxG.sound.play(Paths.sound('salsicha1'));

					case 1296:
						FlxG.camera.flash(FlxColor.WHITE, 2);
						changeCharacter(-380, 1000, 'salsicha-bombado', 'dadOpponent');
					...
					case 2096:
						FlxG.camera.flash(FlxColor.WHITE, 2);
						changeCharacter(-380, 850, 'salsicha', 'dadOpponent');

					case 2273 | 2275 | 2277 | 2279 | 2281 | 2284:
						if (dadOpponent.animOffsets.exists('risada'))
							dadOpponent.playAnim('risada', true);
	...
	// da psych engine, adaptada pra forever.
	...
	// vou só fazer isso logo
	private function doThenDance(anim:String, char:Character, time:Float = null, loop:Bool = false)
	{
		if (char.animOffsets.exists(anim))
			char.playAnim(anim, loop);
	...
	override function beatHit()
	{
		...
		switch(curSong.toLowerCase())
		{
			case 'rap-king':
				if(curBeat >= 728)
					dadOpponent.playAnim('throwmic', true);
			case 'shacklesz':
				if(curBeat >= 288 && curBeat < 317 || curBeat >= 320 && curBeat < 351)
				{
					FlxG.camera.zoom += 0.015;
					camHUD.zoom += 0.03;
					for (hud in strumHUD)
			...
			case 'earthquake':			//eu nao sei se tem uma forma mais pratica para isso então fui desse jeito cringe :(
				if(curBeat >= 196 && curBeat < 224 || curBeat >= 228 && curBeat < 257)
				{
				...
				if(curBeat >= 396 && curBeat < 459 && curBeat >= 520 && curBeat < 519)
				{
					FlxG.camera.zoom += 0.015;
					camHUD.zoom += 0.03;
					for (hud in strumHUD)
		...
		if (!Init.trueSettings.get('Reduced Movements'))
		{
			if (curBeat % 1 == 0)
			{
				uiHUD.iconP1.scale.set(0.9, 0.9);
				uiHUD.iconP2.scale.set(1.1, 1.1);
			}

			if (curBeat % 2 == 0)
			{
				uiHUD.iconP1.scale.set(1.1, 1.1);
				uiHUD.iconP2.scale.set(0.9, 0.9);
			}
		}
		//
	...
	#if desktop
	function startVideo(name:String, ?atend:Bool)
	{
		inCutscene = true;
